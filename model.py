...
class SAASRControl(nn.Module):
    ...
    def forward(
        ...
        cur_speaker = x.tape.split()[0].strip("<>")  # extract first speaker label
        ...
        if x.ai_response is not None:
            ...
            speaker_ai_idx = addressee_to_idx["Assistant"] if addressee_to_idx else None
            speaker_ai_label = torch.tensor(speaker_ai_idx).unsqueeze(0).to(cls.device)
            speaker_ai_embd = self.addressee_embedding(speaker_ai_label)
