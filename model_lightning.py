    ...
    def validation_step(self, batch: TrainBatch, batch_idx):
        ...
        for chunk in sample:
            ...
            try:
                ...
                pred_addressee, pred_ai_addressee, pred_control_token_list = self.check_predictions(
                    addressee,
                    ai_addressee,
                    control_token,
                    chunk.addressee,
        ...
        self.log(f"val_acc_{self.control_token_labels[0]}", batch_acc_control_token[0], prog_bar=True, batch_size=len(sample))
        self.log(f"val_acc_{self.control_token_labels[1]}", batch_acc_control_token[1], prog_bar=True, batch_size=len(sample))
        self.log(f"val_acc_{self.control_token_labels[2]}", batch_acc_control_token[2], prog_bar=True, batch_size=len(sample))
        self.log(f"val_acc_{self.control_token_labels[3]}", batch_acc_control_token[3], prog_bar=True, batch_size=len(sample))
